...
fn part1(name: &str) -> String {
    ...
    for line in contents.iter()
        .take_while(|s| s.len() > 1) {
            println!("{}", line);
            if (&line).len() > 1 { parse_stack(&line, &mut stacks) }
        }

    for (number, from, to) in contents.into_iter().skip_while(|s| (*s).len() > 1).filter_map(parse_move) {
        for _i in 0..number {
            let piece = stacks[from].pop_front().unwrap();
            stacks[to].push_front(piece)
        }
    }
    stacks.iter_mut().map(|s| s.pop_front().unwrap()).collect::<String>()
}
